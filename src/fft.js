...
const Fft = function (radix) {
  ...
  for (let i = fft.sinTable.length; i--;) {
  ...
  const windowFunctions = function (params) {
  ...
  const self = {
    forward: function (b, window) {
      let i, j, k, k2, h, d, c, s, ik, dx, dy
      const n = fft.buffer.length
      const winFunction = {
        name: window,
        N: n,
        a: 0.5,
      ...
      const w = windowFunctions(winFunction)
      if (typeof w === 'number') {
        for (i = 0; i < n; ++i) {
          winFunction.n = i
    ...
    windows: function () {
      const winFuncs = []
      for (const k in windowCalculation) {
        winFuncs.push(k)
      }
      return winFuncs
    }
